...
export class CampusMap implements OnInit {
  ...
  ngOnInit() : void {
    ...
    Crerar.init_bubble('Crerar', 41.7901331, 41.7909298, -87.6031023, -87.6025138)
    this.addToMap(Crerar.id, Crerar)
    var Bookstore = new Bubble()
    Bookstore.init_bubble('Bookstore', 41.7896584,41.7899199,-87.6018721, -87.6013954)
    this.addToMap(Bookstore.id, Bookstore)
    var GeoLab = new Bubble()
    GeoLab.init_bubble('Hinds Lab',41.790033,41.7905151,-87.601902, -87.601432)
    this.addToMap(GeoLab.id, GeoLab)
    var Kovler = new Bubble()
    Kovler.init_bubble('Kovler',41.7896546,41.7901571,-87.6035022, -87.6032333)
    this.addToMap(Kovler.id, Kovler)
    var Cummings = new Bubble()
    Cummings.init_bubble('Cumming Life Center', 41.78973618643269, 41.79001688226163, -87.60298397624403, -87.60253772608442)
    this.addToMap(Cummings.id, Cummings)
    var GordonWest = new Bubble()
    GordonWest.init_bubble('Gordon Center for Integrative Science [West]', 41.79049964901415, 41.79122557913644, -87.60353808709554, -87.60320281099578)
    this.addToMap(GordonWest.id, GordonWest)
    var GordonEast = new Bubble()
    GordonEast.init_bubble('Gordon Center for Integrative Science [East]', 41.791023312732435, 41.79123957770871, -87.60323767969064, -87.6023894680819)
    this.addToMap(GordonEast.id, GordonEast)
    var Frank = new Bubble()
    Frank.init_bubble('James Franck Institute', 41.79076095044874, 41.79124951934684, -87.60238811687489, -87.60188719302559)
    this.addToMap(Frank.id, Frank)
    var Kersten = new Bubble()
    Kersten.init_bubble('Kersten Teaching Center', 41.79074118918219, 41.79125310772268, -87.60187300039753, -87.60143670185623)
    this.addToMap(Kersten.id, Kersten)
  /*===================================================
    ===============  Harper Quadrangle  ===============
    =====================================================*/
    var Cobb = new Bubble()
    Cobb.init_bubble('Cobb Hall', 41.78873284336094,41.789193407299585, -87.60109790155668, -87.60080074965704)
    this.addToMap(Cobb.id, Cobb)
    var Godspeed = new Bubble()
    Godspeed.init_bubble('Godspeed Hall', 41.78803035339906, 41.78872707776446, -87.60104646075854, -87.60088472281471)
    this.addToMap(Godspeed.id, Godspeed)
    var Bond = new Bubble()
    Bond.init_bubble('Bond Chaper', 41.78863584354905, 41.788710971104166, -87.6006170093735, -87.60028175286207)
    this.addToMap(Bond.id, Bond)
    var Swift = new Bubble()
    var Haskell = new Bubble()
    Haskell.init_bubble('Haskell Hall', 41.7881412840618, 41.78862464427493, -87.60016085273766, -87.60000169568687)
    var Weiboldt = new Bubble()
    Weiboldt.init_bubble('Weiboldt Hall', 41.78787764605307, 41.78805759030377, -87.60107505494783, -87.60003025716154)
