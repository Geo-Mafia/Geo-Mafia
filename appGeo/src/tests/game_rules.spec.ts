...
QUnit.test("Constructor and Setting", function(assert) {
    ...
    assert.equal(gameRule.getDayCycleLength(), 86400000, "Default game cycle is 24 hours")
    assert.equal(gameRule.getSafeLength(), 28800000, "Default safe period is 8 hours")
    assert.equal(gameRule.getVoteLength(), 3600000, "Default vote period is 1 hour")
    assert.equal(gameRule.getVoteTime(), 57600000, "Default vote time is 16 hours into game")
    ...
    assert.equal(gameRule.getGameLength(), 90000000, "game length sets correctly")
    assert.equal(gameRule.getDayCycleLength(), 15000000, "game cycle sets correctly")
    assert.equal(gameRule.getSafeLength(), 7500000, "safe period sets correctly")
    assert.equal(gameRule.getVoteLength(), 7500000, "vote period sets correctly")
